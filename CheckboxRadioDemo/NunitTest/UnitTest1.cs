...
namespace Demoproject
{
    class Day3secondsession
    {
        ...
        {
            driver = new ChromeDriver(@"E:\Session\CheckboxRadioDemo\NunitTest\drivers\");
        ...
        public void test()
        {
            IList<IWebElement> RBtn = driver.FindElements(By.Name("sex"));
            RBtn.ElementAt(0);

            bool b = false;

            b = RBtn.ElementAt(0).Selected;

            if (b == true)

            {
                RBtn.ElementAt(1).Click();
                
                Console.WriteLine("Selected gender is " + RBtn.ElementAt(1).GetAttribute("Value"));
            }
            else
                RButton.ElementAt(0).Click();
                Console.WriteLine("Selected gender is " + RBtn.ElementAt(0).GetAttribute("Value"));
